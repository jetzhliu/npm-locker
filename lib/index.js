...
module.exports = function(projectPath, useDev, addCache) {
  ...
  log('total packages count: ' + Object.keys(packages).reduce(function(total, name) {
    return total + Object.keys(packages[name]).length
  }, 0))
...
function copyCache(packages, packageDir, done) {
  ...
  var next = function() {
    ...
    var cachedPath = keys[count]
    var dist = packages[cachedPath]
    if (!fs.existsSync(dist)) {
      exec("cp '" + cachedPath + "' '" + dist + "'", function (err, _, stderr) {
        if (err) {
